...
//Inserting Dummy Users To Database
...
//Deleting Dummy Users From Database
const unSeedDatabase = async () => {
  try {
    await connectDB();

    // Use a filter to delete users by their email addresses
    const emails = seedUsers.map(user => user.email); 
    // Delete users whose emails are in the seedUsers array
